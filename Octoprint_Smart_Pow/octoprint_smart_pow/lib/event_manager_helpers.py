...
def fire_automatic_power_off_do_change_event(event_manager, state: ScheduledPowerOffState):
    event_manager.fire(
        event=Events.AUTOMATIC_POWER_OFF_DO_CHANGE_EVENT(),
        payload=scheduled_power_off_state_to_api_repr(state),
    )

def fire_automatic_power_off_changed_event(event_manager, state: ScheduledPowerOffState):
    event_manager.fire(
