...
class PowerStatePublisher:
    ...
    async def __read_current_state(self,retry_attempts=MAX_CONNECTION_FAILED_RETRY_ATTEMPTS, backoff_seconds=5) -> PowerState:
        """
        ...
        while True:
            try:
                ...
                if attempt == retry_attempts:
                    raise
                self.logger.warning("Error when reading Smart Plug state, retrying after backoff...")
    ...
    async def __publish_if_changed(self):
        ...
        if self.last_updated_state != current_state or self.last_updated_state is None:
